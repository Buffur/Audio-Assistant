...
def _normalized_session_mapping(session: dict[str, Any]) -> dict[str, Any]:
    return ReadingSession.from_mapping(session, now=_now()).to_mapping()


def _prepare_session_defaults(session: dict[str, Any]) -> None:
    normalized_session = _normalized_session_mapping(session)
    session.clear()
...
def _deserialize_session(raw: str | bytes | None) -> dict[str, Any] | None:
    ...
    try:
        return _normalized_session_mapping(value)
...
async def _redis_store_session(user_id: int, session: dict[str, Any]) -> None:
    ...
    await client.setex(
        ...
        SESSION_TTL_SECONDS,
        _serialize_session(normalized_session),
    )
...
async def update_reading_session(user_id: int, **fields: Any) -> None:
    ...
    async with lock:
        ...
        updated_session.update(_updates_with_generation_metadata(fields))
        session.clear()
        session.update(_normalized_session_mapping(updated_session))
        _touch_session(session)
