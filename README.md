...
- TXT-файли;
- фото з текстом;
- JPG/JPEG/PNG як файл для OCR;
- посилання на статтю або новину.

Формати файлів перевіряються не лише за назвою або MIME-типом, а й за вмістом. Інші Office/ZIP-based формати, наприклад PPTX або XLSX, не обробляються як DOCX.
...
| `READING_AUDIO_QUEUE_WORKER_COUNT` | Ні | Кількість audio worker-ів. Для Redis production зазвичай `2+`, з per-user serialization. |
| `EXPORT_AUDIO_CONCAT_TIMEOUT_SECONDS` | Ні | Максимальний час об'єднання частин у повний аудіофайл. |
| `PDF_EXTRACTION_TIMEOUT_SECONDS` / `DOCX_EXTRACTION_TIMEOUT_SECONDS` / `TXT_EXTRACTION_TIMEOUT_SECONDS` | Ні | Explicit timeouts для важкої обробки файлів. |
| `TELEGRAM_WEBHOOK_SECRET_TOKEN` | Так для webhook | Секрет Telegram webhook. Обов'язковий, якщо `BOT_RUNTIME_MODE=webhook`. |
...
- Audio queue має окремий low-priority шлях для prefetch. User-visible jobs (`send_chunk`, `export_audio`) мають пріоритет над prefetch jobs і серіалізуються per-user, щоб не ламати порядок частин.
- Redis audio queue використовує pending/processing списки з requeue при restart/shutdown. Це зменшує ризик втрати задач під час deploy або падіння worker-а.
- Audio cache shared між користувачами, але має ownership index. `/delete_my_data` видаляє тільки кеш, прив'язаний до конкретного користувача, і не інвалідовує кеш інших користувачів.
...
Команда `/catalog_clear` очищає каталог документів. Команда `/delete_my_data` після підтвердження очищає історію документів, персональні налаштування, активну reading-сесію, задачі озвучки користувача в черзі та audio cache, який належить цьому користувачу.
