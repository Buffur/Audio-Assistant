...
Для перегляду або демонстрації кваліфікаційного проєкту спочатку зв'яжіться з автором проєкту. Бот потребує налаштованих секретів (`BOT_TOKEN`, `GEMINI_API_KEY`) і прив'язаний до конкретного Telegram-бота, тому найзручніше показувати його через готовий налаштований екземпляр.
