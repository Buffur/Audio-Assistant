...
TEXT_MESSAGE_LIMIT_REACHED_TEXT = (
    ...
    "Спробуйте завтра або зверніться до адміністратора для отримання Ліміт+."
)
...
OCR_LIMIT_REACHED_TEXT = (
    "🚫 Ви досягли денного ліміту на розпізнавання фото або зображень.\n\n"
    "Спробуйте завтра або зверніться до адміністратора для отримання Ліміт+."
)
...
def build_usage_text(
    ...
    if plan_name == "premium":
        plan_line = f"🎯 Ліміт+ {premium_text}"
    else:
        plan_line = "🎯 Базовий ліміт"
    ...
    return (
        ...
        f"🖼 Фотографії: {ocr_used} / {_format_limit_value(ocr_limit)}\n"
...
def build_premium_granted_text(user_id: int, days: int | None, premium_until: str | None) -> str:
    if days is None:
        return f"✅ Користувачу {user_id} видано Ліміт+ безстроково."

    return (
        f"✅ Користувачу {user_id} видано Ліміт+ на {days} днів.\n"
        f"Діє до: {premium_until}"
    )


def build_premium_revoked_text(user_id: int) -> str:
    return f"✅ Ліміт+ у користувача {user_id} скасовано."
...
def build_premium_status_text(
    ...
    if plan_name == "premium":
        return (
            f"💎 Користувач {user_id}: Ліміт+\n"
            f"Діє до: {premium_until or 'безстроково'}"
        )

    return f"🎯 Користувач {user_id}: Ліміт"
